...
| 44 | Medium | `--task <file>` on a non-UTF-8 file crashed with a raw `UnicodeDecodeError` traceback instead of a clean exit-1 error message, unlike the symmetric missing-file path which prints a one-line error | v1.4 (explicit `read_text(encoding="utf-8")` + `UnicodeDecodeError` catch mirroring the file-not-found error shape) |
| 45 | Medium | `container.send_keys` / `capture_visible` / `capture_full_history` let raw `subprocess.CalledProcessError` (from `docker exec` failing because the container / session went away mid-run) propagate through `shell._apply_send_keys` — that handler's `except ContainerError:` didn't match, so the error escaped `agent.run`, dumped a traceback to stderr, and left the run in `status="active"` in the DB (the `finally` block still destroyed the container but never updated the run metadata to "failed") | v1.4 (container-level funcs catch `CalledProcessError` → raise `SessionDeadError` so shell's existing `ContainerError` handler emits `SendKeysFailed` → decide marks run "failed" cleanly) |
